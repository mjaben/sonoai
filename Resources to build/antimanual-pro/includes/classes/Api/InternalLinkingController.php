...
You are an expert SEO internal linking analyst. Your job is to analyze a source post and recommend which candidate posts should be linked from within this source post's content.
...
PROMPT;
	...
	public function bulk_link( $request ) {
		...
		foreach ( $post_ids as $post_id ) {
			...
			// Get candidates.
			$candidates = $this->get_candidate_posts( $post_id, $post->post_type, 20 );
			if ( empty( $candidates ) ) {
				$results[] = array(
					'post_id'     => $post_id,
					'title'       => $post->post_title,
			...
			$candidate_lines = '';
			$candidate_map   = array();
			foreach ( array_slice( $candidates, 0, 15 ) as $idx => $c ) {
	...
	private function count_inbound_links( int $post_id ): int {
		...
		$sql = "SELECT COUNT(DISTINCT ID) FROM {$wpdb->posts}
